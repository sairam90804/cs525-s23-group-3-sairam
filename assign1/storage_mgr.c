...
RC createPageFile (char *filename) {
    ...
    if(file != NULL) { 
		...
		char c;
		//ask to overwrite if file not empty
        printf("Want to replace the content?(y/n)"); 
        scanf("%c",&c);
		if(c == 'y' || 'Y') {
        	...
        	RC_message="You chose not to overwrite the file. Exiting the File";
        	return RC_FILE_EXISTS;
    	}
    }
    else { // create file with new content
        file=fopen(filename,"w");
        char *pointer=(char *)calloc(PAGE_SIZE,sizeof(char));
        fwrite(pointer,PAGE_SIZE,sizeof(char),file);
        fseek(file,0,SEEK_END);
...
RC destroyPageFile (char *filename) {
	//if file is there, remove the file
