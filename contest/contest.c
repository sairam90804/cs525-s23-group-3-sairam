...
main (void)
{
  ...
  PRINTF_AND_FLUSH("\n-------------------------------\n-- WORKLOAD 2 TOTAL: %f sec, I/Os: %ld\n\n", curTime, ios);
...
singleWorkload1 (int size, int numRequests, int numPages, double *t, long *ios)
{
  ...
  PRINTF_AND_FLUSH("\nWORKLOAD 1 - SETUP: N(R)=<%d>, #scans=<%i>, M=<%i>\n", size, numRequests, numPages);
  ...
  (*ios) += getContestIOs();
  PRINTF_AND_FLUSH("\nWORKLOAD 1 - FINISHED - N(R)=<%i>, #scans=<%i>, M=<%i>: %f sec, %ld TOTAL I/Os so far\n", size, numRequests, numPages, *t, *ios);
...
singleWorkload2 (int size, int numRequests, int numPages, int percInserts, int scanFreq, double *t, long *ios)
{
  ...
  PRINTF_AND_FLUSH("\nWORKLOAD 2 - SETUP: N(R)=<%d>, #requests=<%d>, M=<%d>, perc-inserts: <%d>, scan-frequency: <%d>\n", size, numRequests, numPages, percInserts, scanFreq);
  // set up
  srand(0);
  TEST_CHECK(setUpContest(numPages));
  ...
  (*t) += difftime(endTime,startTime);
  (*ios) += getContestIOs();
  PRINTF_AND_FLUSH("\nWORKLOAD 2 - FINISHED - N(R)=<%i>, #scans=<%i>, M=<%i>: %f sec, %ld I/Os\n", size, numRequests, numPages, *t, *ios);
